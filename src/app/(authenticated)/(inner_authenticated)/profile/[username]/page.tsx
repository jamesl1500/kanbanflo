...
export default async function UserProfilePage({ params }: ProfilePageProps) {
    ...
    const tasks = (cardRows ?? [])
        .filter((card) => card.assignee_id === profile.id)
        .slice(0, 12)
        .map((card) => ({
            id: card.id,
            title: card.title,
    ...
    const tasksDoneCount = (cardRows ?? []).filter((card) => card.assignee_id === profile.id && card.status === 'done').length;
    const workspaceCount = (workspaceRows ?? []).length;
    const collaboratorCount = new Set(
        (membersByWorkspace.data ?? [])
            .map((row) => row.user_id)
            .filter((memberUserId) => memberUserId !== profile.id)
    ).size;
            ...
            <div className={styles.profileMainPageInner}>
                ...
                <div className={styles.profileStats}>
                    ...
                    <div className={styles.profileStatCard}>
                        ...
                        <div className={styles.profileStatLabel}>Workspace Peers</div>
                    </div>
                    <div className={styles.profileStatCard}>
